                  ...
                  <article key={post_idx} className="cell post">
                    <div className="card">
                      ...
                      <Link className="post-thumbnail" href={withPrefix(_.get(post, '__metadata.urlPath', null))} title={_.get(post, 'frontmatter.thumb_image_alt', null)}>
                        <img src="/images/placeholder.svg" loading="lazy" data-src={withPrefix(_.get(post, 'frontmatter.thumb_image', null))} alt={_.get(post, 'frontmatter.thumb_image_alt', null)} />
                      </Link>
                      )}
                      <div className="post-body">
                        <header className="post-header">
                          <h3 className="post-title"><Link href={withPrefix(_.get(post, '__metadata.urlPath', null))} title={_.get(post, 'frontmatter.title', null)}>{_.get(post, 'frontmatter.title', null)}</Link></h3>
                        </header>
